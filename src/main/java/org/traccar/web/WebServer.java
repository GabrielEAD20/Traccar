...
public class WebServer implements LifecycleObject {
    ...
    private void initApi(ServletContextHandler servletHandler) {
        ...
        // Condicional para activar/desactivar el filtro de JWT
        boolean isJwtEnabled = true ; //config.getBoolean("security.jwt.enabled");
        if (isJwtEnabled) {
            resourceConfig.register(SecurityRequestFilter.class);
        } else {
            LOGGER.info("JWT security is disabled");
        }
